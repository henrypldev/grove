...
export function dbListAgentsByTeam(teamId: string): Agent[] {
	const rows = getDb().query<DbAgent, [string]>(
...
export function dbGetAgent(id: string): Agent | null {
	const row = getDb().query<DbAgent, [string]>('SELECT * FROM agents WHERE id = ?').get(id)
...
export function dbInsertAgent(agent: Agent): void {
	getDb().run(
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		[agent.id, agent.teamId, agent.role, agent.status, agent.currentTask, agent.sessionId, agent.retryCount, agent.spawnedAt, agent.updatedAt],
	)
}

export function dbUpdateAgentStatus(id: string, status: AgentStatus, currentTask?: string | null): void {
	const now = Date.now()
	getDb().run(
...
export function dbIncrementAgentRetry(id: string): number {
	...
	const row = getDb().query<{ retry_count: number }, [string]>(
