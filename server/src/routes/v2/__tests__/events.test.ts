...
describe('handleV2Events', () => {
	beforeEach(() => {
		makeTestDb()
		dbInsertRepo({ id: 'r1', name: 'repo', path: '/p', envVars: undefined, setupSteps: undefined })
	...
	test('POST /v2/events inserts and returns event', async () => {
		const res = await handleV2Events(
			makeReq('POST', { teamId: 't1', agentId: 'a1', type: 'test', payload: { x: 1 } }),
	...
	test('POST /v2/events unknown team returns 404', async () => {
		const res = await handleV2Events(
			makeReq('POST', { teamId: 'nope', agentId: 'a1', type: 't', payload: {} }),
			new URL('http://localhost/v2/events'),
			headers,
		)
	...
	test('POST /v2/events unknown agent returns 404', async () => {
		const res = await handleV2Events(
			makeReq('POST', { teamId: 't1', agentId: 'nope', type: 't', payload: {} }),
			new URL('http://localhost/v2/events'),
			headers,
		)
		expect(res?.status).toBe(404)
	...
	test('unmatched path returns null', async () => {
		const res = await handleV2Events(makeReq('GET'), new URL('http://localhost/v2/other'), headers)
		expect(res).toBeNull()
	})
})
